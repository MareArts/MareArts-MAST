...
bool MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching();


int main()
{
    MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching();
    return 0;
}
...
bool MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching() {
    ...
    while (1)
    {
        ...
        if (!stitching_done) {
            ...
            MAST2.loadCameraParams("./params/save_param_01");
            MAST2.m_use_GPU = false; //true;
            stitching_done = true;
            ...
            if (MAST2.m_use_GPU == false) {
                //cpu mode
                MAST2.stitcher(frames, result_st);
            }
            else if (MAST2.m_use_GPU == true) {
                //gpu mode
                MAST2.stitcher(frames_gpu, result_st_gpu);
