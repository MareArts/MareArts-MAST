@echo off
setlocal
echo Compiling 4_MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching.cpp...
...
rem Compile the program (using /Fe for output)
cl 4_MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching.cpp /Fe4_MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching.exe ^
   /I "%OPENCV_INCLUDE%" ^
   /I "%MAST_S2_DLL_PATH%" ^
   /EHsc ^
   /link ^
...
rem Run the application in a separate CMD session with a minimal PATH.
rem The minimal PATH includes:
rem   - Custom DLLs from MAST_S2_DLL_PATH
rem   - OpenCV binaries
rem   - Intel TBB DLLs
rem   - CUDA binaries (for cudart64, nppc64, etc.)
rem   - Windows system DLLs
cmd /c "set PATH=%MAST_S2_DLL_PATH%;C:\opencv\x64\vc17\bin;%TBB_DLL_PATH%;%CUDA_BIN%;C:\Windows\system32 && 4_MAST_Aclass_calibration_saveParam_N_loadParam_Bclass_stitching.exe"
