@echo off
setlocal
echo Compiling 1_MAST_calibration_N_stitching.cpp...
...
cl 1_MAST_calibration_N_stitching.cpp /Fe1_MAST_calibration_N_stitching.exe ^
   /I "%OPENCV_INCLUDE%" ^
   /I "%MAST_S2_DLL_PATH%" ^
   /EHsc ^
   /link ^
...
rem Here we include only the folders that should contain the required DLLs:
rem   - Your custom DLLs (MAST_S2_DLL_PATH)
rem   - OpenCV binaries
rem   - Intel TBB DLLs
rem   - CUDA binaries (for cudart64, nppc64, nppial64, nppidei64, etc.)
rem   - System32 as an extra safety net
cmd /c "set PATH=%MAST_S2_DLL_PATH%;C:\opencv\x64\vc17\bin;%TBB_DLL_PATH%;%CUDA_BIN%;C:\Windows\system32 && 1_MAST_calibration_N_stitching.exe"
