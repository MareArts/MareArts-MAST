@echo off
setlocal
echo Compiling 2_MAST_calibration_N_saveParam.cpp...
...
rem Compile the program (using /Fe instead of -o)
cl 2_MAST_calibration_N_saveParam.cpp /Fe2_MAST_calibration_N_saveParam.exe ^
   /I "%OPENCV_INCLUDE%" ^
   /I "%MAST_S2_DLL_PATH%" ^
   /EHsc ^
   /link ^
...
rem This minimal PATH includes only directories with required DLLs:
rem  - Custom DLLs from MAST_S2_DLL_PATH
rem  - OpenCV binaries
rem  - Intel TBB DLLs
rem  - CUDA binaries (for cudart64, nppc64, etc.)
rem  - Windows system DLLs from system32
cmd /c "set PATH=%MAST_S2_DLL_PATH%;C:\opencv\x64\vc17\bin;%TBB_DLL_PATH%;%CUDA_BIN%;C:\Windows\system32 && 2_MAST_calibration_N_saveParam.exe"
