...
echo Compiling T2_bestpractice_test.cpp...
...
if not exist T2_bestpractice_test.cpp (
    echo ERROR: Source file T2_bestpractice_test.cpp not found!
    exit /b 1
)

rem Compile the program
cl T2_bestpractice_test.cpp /Fe:T2_bestpractice_test.exe ^
   /I "%OPENCV_INCLUDE%" ^
   /I "%CUDA_INCLUDE_PATH%" ^
   /I "%MAST_S2_DLL_PATH%" ^
   /EHsc /MD ^
...
echo Running T2_bestpractice_test.exe...
cmd /c "set PATH=%MAST_S2_DLL_PATH%;C:\opencv\x64\vc17\bin;%TBB_DLL_PATH%;%CUDA_BIN%;C:\Windows\system32 && T2_bestpractice_test.exe"
