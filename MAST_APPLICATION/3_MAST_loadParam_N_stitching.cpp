...
bool MAST_loadParam_N_stitching();


int main()
{
    MAST_loadParam_N_stitching();
    return 0;
}
...
bool MAST_loadParam_N_stitching() {
    ...
    //load params
    if(MAST.loadCameraParams("./params/save_param") == false)
        return false;
    MAST.m_use_GPU = true; //true
    MAST.log_out = true;
    //st window init
    ...
    while (1)
    {
        ...
        std::cout << result_st.size() << std::endl;
