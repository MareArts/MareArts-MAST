@echo off
setlocal
echo Compiling 6_best_practice_video_thread_gpuOnly_MAST_calibration_N_stitching.cpp...
...
cl 6_best_practice_video_thread_gpuOnly_MAST_calibration_N_stitching.cpp /Fe6_best_practice_video_thread_gpuOnly_MAST_calibration_N_stitching.exe ^
   /I "%OPENCV_INCLUDE%" ^
   /I "%CUDA_INCLUDE_PATH%" ^
   /I "%MAST_S2_DLL_PATH%" ^
   /EHsc ^
...
rem   - MAST_S2_DLL_PATH (custom DLLs)
rem   - OpenCV binaries (from C:\opencv\x64\vc17\bin)
rem   - Intel TBB DLLs
rem   - CUDA binaries (for cudart64, etc.)
rem   - Windows system DLLs (from system32)
cmd /c "set PATH=%MAST_S2_DLL_PATH%;C:\opencv\x64\vc17\bin;%TBB_DLL_PATH%;%CUDA_BIN%;C:\Windows\system32 && 6_best_practice_video_thread_gpuOnly_MAST_calibration_N_stitching.exe"
