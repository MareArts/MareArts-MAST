@echo off
setlocal
echo Compiling 3_MAST_loadParam_N_stitching.cpp...
...
cl 3_MAST_loadParam_N_stitching.cpp /Fe3_MAST_loadParam_N_stitching.exe ^
   /I "%OPENCV_INCLUDE%" ^
   /I "%MAST_S2_DLL_PATH%" ^
   /EHsc ^
   /link ^
...
rem This minimal PATH includes:
rem   - MAST_S2_DLL_PATH for your custom DLLs
rem   - OpenCV binaries
rem   - Intel TBB DLLs
rem   - CUDA binaries (for cudart64, etc.)
rem   - Windows system DLLs from system32
cmd /c "set PATH=%MAST_S2_DLL_PATH%;C:\opencv\x64\vc17\bin;%TBB_DLL_PATH%;%CUDA_BIN%;C:\Windows\system32 && 3_MAST_loadParam_N_stitching.exe"
